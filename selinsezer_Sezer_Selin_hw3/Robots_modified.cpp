...
/**************** START: CS201 students' hw5 member functions ********************/ 
	...
	int Robot :: GetDeaths ()			
	{
		...
		{
			SetDeath++;
			// It is useful for ShowMessage at the end of the program.	
		}
		else if(!stalled)
		{
			return SetDeath;
	...
	void Robot::Teleport(int x, int y, Direction d)
{
	...
	void Robot :: Resurrect()			// we will use it to keep opponents alive even they face to wall		
	{
		if (stalled =true)
			stalled = false;
			theRobotWindow->Redraw(this);
	...
	int Robot:: GetBagCount() const
	{
		return bag;
	}

	void Robot :: SetBagCount(Robot & w)		// It helps me to reduce bag things, when it turns to a superrobot
	{
		bag=w.GetBagCount()-10;
		theRobotWindow->Redraw(this);
		}
	void Robot :: SetOtherBagCount(Robot & w)		// It helps me to reduce bag things, when it turns to a superrobot
	{
		bag=w.GetBagCount()+10;
		theRobotWindow->Redraw(this);
		}
	
	void Robot :: KillRobots()					//  kill player robot
	{
		if (stalled = false)
		{
			stalled = true;
